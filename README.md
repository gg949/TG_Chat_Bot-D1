...
**Telegram 双向机器人 Cloudflare Worker 混合验证版** 带来了质的飞跃：完美继承私聊消息转发到群组话题的 CRM 核心能力，更引入 **三模态混合验证系统**（Cloudflare/Google/关闭）与 **独立问题/图像验证** 机制，配合全新可视化协管管理，为您提供企业级私聊托管与风控解决方案。
...
| 特性 | 说明 |
| :--- | :--- |
| 🛡️ **混合验证架构** | 支持 Cloudflare Turnstile、Google reCAPTCHA 热轮换，内置原生 SVG 算式图形验证码与安全问答（Q&A），全方位抵御机器人刷屏 |
| 🧩 **多重风控流转** | 外部验证码、图形验证与问答验证完全解耦，支持自由叠加，用户需依序通过开启项方可与客服建联 |
| 💬 **双向客服会话** | 基于 Telegram 论坛话题（Forum Topics）实现单人单卡与独立线程会话隔离，支持实时双向转发沟通 |
| 🗑️ **即焚与双向撤回** | 支持关闭群话题自动销毁 Bot 消息，或通过 `/recall` 批量撤回 48 小时内发出的全部回复，彻底不留痕迹 |
| 👍 **已送达交互回执** | 消息成功转入后台话题后，自动给用户端原消息贴上指定的 Reaction 表情回执，状态反馈直观即时 |
| ✏️ **修改实时追踪** | 用户私聊修改文字或媒体说明时，管理群对应话题即刻同步推送修改前后记录，防止信息篡改 |
| 🔔 **未读聚合与自愈** | 自动创建 `🔔 未读消息` 聚合看板，支持一键直达回复与销毁卡片；话题被删时可在转发链路中无感自动重建 |
| 👮 **分级协管协同** | 主管理员专属控制台，协管员直接通过面板增删纯数字 ID，享有话题回复、改备注与撤回等日常客服权限 |
| 🛠️ **多维消息过滤** | 支持独立拦截转发、媒体、语音、贴纸、链接等消息类型，并集成 ReDoS 缓解与违禁词阈值自动封禁机制 |
| 🌙 **营业管理与备份** | 支持一键切换营业/休息模式并配置防刷节流回复语，支持绑定独立外部备份群组实现数据实时容灾 |
